...
function c511009365.tgtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c511009365.tgfilter(chkc,tp) end
	if chk==0 then return Duel.IsExistingTarget(c511009365.tgfilter,tp,LOCATION_MZONE,0,1,e:GetHandler(),tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,c511009365.tgfilter,tp,LOCATION_MZONE,0,1,1,e:GetHandler(),tp)
end
