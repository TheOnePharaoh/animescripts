...
function self.initial_effect(c)
  ...
  if not self['gl_chk'] then
    ...
    ge1:SetOperation(self.gl_op)
    Duel.RegisterEffect(ge1,0)
  end
  --Oath
  Duel.AddCustomActivityCounter(511005064,ACTIVITY_SUMMON,self.oath_fil)
  Duel.AddCustomActivityCounter(511005064,ACTIVITY_SPSUMMON,self.oath_fil)
end

function self.trap_fil(c)
  return c:IsCode(511005064) and c:IsFaceup() and c:GetFlagEffect(511005064)==0
end

function self.gl_op(e,tp,eg,ep,ev,re,r,rp)
  if Duel.GetAttacker():IsType(TYPE_FUSION) and not Duel.GetAttackTarget() then
    local g=Duel.GetMatchingGroup(self.trap_fil,tp,LOCATION_SZONE,LOCATION_SZONE,nil)
...
function self.ca_tg(e,c)
  return not c:IsType(TYPE_FUSION)
end

function self.da_tg(e,c)
