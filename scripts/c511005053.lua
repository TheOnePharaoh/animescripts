...
function self.initial_effect(c)
  ...
  e3:SetCondition(self.dm_cd)
  e3:SetTarget(self.dm_tg)
  e3:SetOperation(self.dm_op)
  c:RegisterEffect(e3)
end
...
function self.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  if chkc then return chkc:GetLocation()==LOCATION_MZONE and self.fil(chkc) end
  if chk==0 then return Duel.IsExistingTarget(self.fil,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
  Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
  Duel.SelectTarget(tp,self.fil,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
