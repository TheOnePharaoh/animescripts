...
function c511009344.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	aux.AddFusionProcCodeFun(c,6205579,c511009344.ffilter,1,true,true)
	-- atk up
...
function c511009344.atkval(e,c)
	return c:GetEquipGroup():FilterCount(Card.IsCode,nil,6205579)*-800
end
function c511009344.condition(e,tp,eg,ep,ev,re,r,rp)
	return tp~=Duel.GetTurnPlayer() and e:GetHandler():GetEquipGroup():IsExists(Card.IsCode,1,nil,6205579)
end


function c511009344.operation(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetAttacker()
	local eqc=e:GetHandler():GetEquipGroup():Filter(Card.IsCode,nil,6205579):GetFirst()
